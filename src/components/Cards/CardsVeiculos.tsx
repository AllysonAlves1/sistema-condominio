...
export default function CardsVeiculos() {
  ...
  useEffect(() => {
    const countEntrada = async () => {
      await axios.get("http://localhost:3000/acessoveiculo/countAcessoEntrada").then((response) => {
    ...
    const countSaida = async () => {
      await axios.get("http://localhost:3000/acessoveiculo/countAcessoSaida").then((response) => {
